...
export default function MangaDetailCommentSection({ mangaId, mangaTitle = "Unknown Manga" }) {
  ...
  const COMMENT_COOLDOWN = 60 * 1000; // 1 minute cooldown
  ...
  const fetchComments = async () => {
    const { data, error } = await supabase
      .from("manga_detail_comments") // New table for manga detail comments
      .select("*")
      .eq("manga_id", mangaId)
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel("manga_detail_comments")
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "manga_detail_comments" }, fetchComments)
      .subscribe();
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      if (replyingTo) {
        const { data: parentComment } = await supabase.from("manga_detail_comments").select("user_id").eq("id", replyingTo).single();
        if (parentComment?.user_id) {
          await sendNotification(parentComment.user_id, `${user.name} replied to your comment on "${mangaTitle}".`);
        }
      }

      await sendNotification(user.id, `Your comment on "${mangaTitle || "a manga"}" was posted successfully.`);
        ...
        await supabase.from("wallet_events").insert([
          {
            ...
            event_details: "manga_detail_comment_reward",
